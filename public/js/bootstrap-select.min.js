        ...
        selectPosition: function () {
          ...
          var s,
            ...
            e = function (e) {
              var t = {},
                i =
                  l.options.display || F.fn.dropdown.Constructor.Default.display
              l.$bsContainer
                .addClass(
...
/* Select Box With Search Options Example for home area*/
;(function ($) {
  /*
   * Select box with search function
   *
  ...
  $.fn.searchBox = function (opts) {
    ...
    var init = function (obj) {
      var self = $(obj),
        ...
        searchWord = '' // Filter string
      // Addition of text input field for refined search
      ...
      // If selected in the initial display, the selected wording is displayed in the narrowed wording input field.
      ...
      // Create a dummy list to display instead of a select box
      var visibleTarget = self
        .find('option')
        .map(function (i, e) {
          return (
            '<li data-selected="off" data-searchval="' +
            $(e).val() +
            '"><span>' +
            $(e).text() +
            '</span></li>'
      ...
      var refineTextWidth = settings.elementWidth
      ...
      self.hide()
      // Narrow down the dummy list by search conditions.
      var changeSearchBoxElement = function () {
        if (searchWord !== '') {
          var matcher = new RegExp(searchWord.replace(/\\/g, '\\\\'), 'i')
          var filterTarget = $(visibleTarget.join()) // Copy of array
          ...
          parent.find('.searchBoxElement').empty()
          parent
            .find('.searchBoxElement')
            .html(visibleTarget.slice(0, settings.optionMaxSize).join(''))
          parent.find('.searchBoxElement').show()
        }
        // Changes the background color of the selected LI tag.
        var selectedOption = self.find('option:selected')
        if (selectedOption) {
          parent
            .find('.searchBoxElement')
          ...
          parent
            .find('.searchBoxElement')
            .find('li[data-searchval="' + selectedOption.val() + '"]')
            .addClass('selected')
        }
        // When a dummy list is selected
        parent
          .find('.searchBoxElement')
          .find('li')
          .on('click', function (e) {
      ...
      // keyupFunction of time
      refineText.keyup(function (e) {
        searchWord = $(this).val()
        // Refresh the dummy list
        changeSearchBoxElement()
      })
      // When changing the select box
      self.change(function () {
        // Reflect the text of the selection option in the latest refined wording area
      ...
      // If you click the text box, a dummy list will be displayed.
      ...
      // If you click outside the select box, the dummy list will be hidden.
  ...
  var MODE = {
    NORMAL: 0, // Normal select box
    INPUT: 1, // Input type select box
    TAG: 2 // Tag addition type select box
  }
  $.fn.searchBox.defaults = {
    selectCallback: null, // Callback called after selection
    elementWidth: null, // Display width of select box
    optionMaxSize: 100, // Maximum number to display in the select box
    mode: MODE.INPUT // Display mode
  }
})(jQuery)
